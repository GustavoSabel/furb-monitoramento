...
$(function() {
	webSocketManager = new WebSocketManager(aoConectar, aoReceberMensagem,
			aoDesconectar, aOcorrerErro);
					...
					function(data) {
						...
						data
								.forEach(function(row) {
									linhas += "<tr class='"
											+ DISP_NAO_ENCONTRATO + "' id='"
											+ row[1].macToId() + "'>";
									linhas += "<td class='macaddress'	>"
											+ row[1] + "</td>";
									linhas += "<td class='status'		> # </td>";
									linhas += "<td class='ip'			> # </td>";
									linhas += "<td class='local'		>" + row[2]
											+ "</td>";
									linhas += "<td class='observacao'	>"
											+ row[3] + "</td>";
									linhas += "<td class='comandos'>";
									linhas += "<input type='checkbox' name='relay' value='relay' onchange='toggleRelay(this, \""
											+ row[1] + "\")'>Relay</input>";
									linhas += "<button onClick='Desligar(\""
											+ row[1] + "\")'>Desligar</button>";
									linhas += "</td>";
...
function buscarIps() {
	...
	AjustarStatusLinhas(DISP_NAO_ENCONTRATO, DISP_NAO_ENCONTRATO,
			"Procurando...")
...
function aoReceberMensagem(event, socket) {
	...
	if (resultado["EventURL"] == "/mod-io2") {
		 ...
		 * socket.macAddress.macToId()); var celula =
		 * linha.querySelector("td.sensor"); celula.firstChild.textContent = ;
		 */
...
function aoDesconectar(event, socket) {
	AtualizarStatus(socket.macAddress, DISP_NAO_CONECTADO, "Desconectou...",
			null);
}

function aOcorrerErro(event, socket) {
	AtualizarStatus(socket.macAddress, DISP_NAO_CONECTADO, "Erro na conexão",
			null);
}
...
function aoConectar(sucesso, socket) {
	if (sucesso) {
		...
		AtualizarStatus(socket.macAddress, DISP_NAO_CONECTADO, "Erro conexão",
				null);
	}
}
...
function getLinhaByIP(ip) {
	return document.querySelector("#"
			+ dispositivos[ip]["macAddress"].macToId());
}
